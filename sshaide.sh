...
#       SSH authentication with a public/private password-less key pair
#       to perform automated, scripted access to a remote system.  Naturally
...
#       recommended that sshaide.sh use non-privileged accounts and the
#       key installed on the client be configured to be restricted for use
#       by specific sshaide manager stations.  While non-privileged access
#       this limits our ability to verify some files and diretories on
#       remote systems, we believe it is an acceptable trade-off.  Most
#       critical files and directories can be effectively monitored without
#       having  privileged access.  It is recommended that an unprivileged,
#       but dedicated account on the manager station also be setup to manage
#       AIDE databases, AIDE reports, remote logins and other sshaide.sh
#       requirements.
#
#       Remote clients must have the public SSH key that will be used by
#       the sshaide.sh manager.  The sshaide.sh manager must have the
...
# 2017-04-18,jtk: moving to github, minor doc edits, preparing major rev
